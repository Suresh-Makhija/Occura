...
public class AllLiveListDAO {
	...
	//List Of all Status
	public List<StatusBean> listOfStatus() {
		Session session = HibernateUtilLiveDatabase.openSession();
		Transaction tx = null;
		...
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from StatusBean");
			listOfStatus = query.list();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
	...
	public List<ProductMasterBean> listOfProduct() {
		...
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from ProductMasterBean");
	...
	public List<ProductProcessBean> listOfProductProcess() {
		Session session = HibernateUtilLiveDatabase.openSession();
		Transaction tx = null;
		List<ProductProcessBean> listOfProductProcess = new ArrayList<ProductProcessBean>();
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from ProductProcessBean ");
			listOfProductProcess = query.list();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
	...
	public List<WorkProductProcessBean> listOfWorkproductProcess(String custId) {
		...
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from WorkProductProcessBean where workHistoryBean.customerBean.parent_customer_id = '"+custId+"' and insert_datetime >= DATEADD(DAY,-3,GETDATE()) order by insert_datetime DESC");
	...
	public List<WorkHistoryBean> listOfWorkHistory(String custId) {
		...
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from WorkHistoryBean where customerBean.parent_customer_id = '"+custId+"' and insert_date >= DATEADD(DAY,-30,GETDATE())");
	...
	public List<WorkProductBean> listOfWorkProduct(String custId) {
		...
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from WorkProductBean where workHistoryBean.customerBean.parent_customer_id = '"+custId+"' and insert_datetime >= DATEADD(DAY,-30,GETDATE())");
			listOfWorkHistory = query.list();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
	...
	public List<CustomerTypeBean> listOfCustomerType() {
		Session session = HibernateUtilLiveDatabase.openSession();
		Transaction tx = null;
		List<CustomerTypeBean> listOfCutomerType = new ArrayList<CustomerTypeBean>();
		try {
			tx = session.getTransaction();
			tx.begin();
			Query query = session.createQuery("from CustomerTypeBean");
			listOfCutomerType = query.list();
			tx.commit();
		} catch (Exception e) {
			if (tx != null) {
				tx.rollback();
	...
	public List<ChallanOutLineBean> listOfChallanOutLine(String custId) {
		Session session = HibernateUtilLiveDatabase.openSession();
		Transaction tx = null;
		List<ChallanOutLineBean> listOfChallanLine = new ArrayList<ChallanOutLineBean>();
		try {
			tx = session.getTransaction();
			tx.begin();
