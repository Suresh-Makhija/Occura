...
function download_from_local(a) {
    ...
    }, e.open("GET", a, !0), e.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), e.responseType = "arraybuffer", e.send(null)
}
...
function after_file_load(a) {
    ...
    if (a) {
        try {
            e = parse_3d_file(filename, a)
        } catch (a) {
            e = "Error parsing the file"
        }
        "string" != typeof e ? postMessage({
...
self.addEventListener("message", function(a) {
    switch (a.data.msg_type) {
        case MSG_DATA:
            ...
            a.data.data.local_file ? (filename = a.data.data.local_file.name, local_file = a.data.data.local_file ? a.data.data.local_file : null) : a.data.data.filename && (filename = a.data.data.filename), a.data.data.x && isNumeric(a.data.data.x) && (x = a.data.data.x), a.data.data.y && isNumeric(a.data.data.y) && (y = a.data.data.y), a.data.data.y && isNumeric(a.data.data.z) && (z = a.data.data.z), model_id = a.data.data.id ? a.data.data.id : -1, get_progress = !!a.data.get_progress && a.data.get_progress;
            break;
        case MSG_LOAD:
            local_file ? read_file(local_file) : filename && download_from_local(filename);
            break;
        default:
